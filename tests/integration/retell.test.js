...
describe('Retell AI webhook', () => {
  ...
  it('inserts a new urgencia per call_id and does not overwrite by phone', async () => {
    ...
    assert.match(String(rows.rows[0].summary || ''), /El cliente Ana Primera solicitó asistencia urgente/);
    assert.match(String(rows.rows[0].summary || ''), /Seat Ibiza/);
    assert.match(String(rows.rows[0].summary || ''), /Pinchazo/);
    assert.match(String(rows.rows[1].summary || ''), /El cliente Pedro Segundo solicitó asistencia urgente/);
