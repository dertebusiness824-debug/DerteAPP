...
describe('home brand-blue theme', () => {
  it('keeps home action bindings and the split dashboard markup', () => {
    ...
    assert.match(view, /spinTriggerMark/);
    assert.match(view, /clearTriggerSpin\(swap, \{ restart: true \}\)/);
    assert.match(view, /spinTriggerMark/);
    assert.doesNotMatch(view, /is-tool/);
    assert.doesNotMatch(view, /home-split__trigger-wrench/);
    assert.doesNotMatch(view, /icon\('wrench'/);
    ...
    assert.match(i18n, /'home\.trustTagline':\s*'TODO EN UNO'/);
    assert.doesNotMatch(i18n, /🛠️/);
  ...
  it('paints Inicio over a workshop photo with a readable white overlay', () => {
    ...
    assert.doesNotMatch(css, /@keyframes home-glyph-in/);
    assert.doesNotMatch(css, /@keyframes home-glyph-out/);
    ...
    assert.doesNotMatch(css, /home-split__trigger-wrench/);
    assert.doesNotMatch(css, /\.home-split__trigger\.is-tool/);
    ...
    assert.match(css, /\.home-split__tile::after[\s\S]*0 0 15px var\(--home-neon-cyan\)/);
    assert.match(css, /\.home-split__tile--urgencias::after\s*\{[^}]*0 0 15px var\(--home-neon-red\)/s);
    assert.match(css, /\.home-split__tile--diagnostics::after\s*\{[^}]*0 0 15px var\(--home-neon-cobalt\)/s);
    assert.match(css, /\.home-split__tile--pending::after\s*\{[^}]*0 0 15px var\(--home-neon-orange\)/s);
    assert.match(css, /\.home-split__kpi--done::after\s*\{[^}]*0 0 15px var\(--home-neon-green\)/s);
    assert.match(css, /\.home-split__kpi--pending::after\s*\{[^}]*0 0 15px var\(--home-neon-orange\)/s);
    assert.match(css, /@keyframes home-tile-enter[\s\S]*translate3d\(0, 20px, 0\) scale\(0\.95\)/);
    assert.match(css, /@keyframes home-tile-enter[\s\S]*translate3d\(0, 0, 0\) scale\(1\)/);
    assert.match(css, /@keyframes home-neon-enter[\s\S]*72%/);
    assert.match(css, /\.home-split__tile:nth-child\(2\)[\s\S]*animation-delay:\s*calc\(var\(--home-stagger\) \* 1\)/);
    assert.match(css, /\.home-split__tile\s*\{[^}]*will-change:\s*transform, opacity/s);
    assert.match(css, /\.home-split__tile::after[\s\S]*will-change:\s*opacity/);
    assert.doesNotMatch(css, /\.home-split__tile\s*\{[^}]*transition:[^}]*box-shadow/s);
