...
router.get('/retell', (_req, res) => {
  res.json({
    provider: 'retell',
    ready: true,
    signature_verification: config.retell.verifyWebhooks ? (config.retell.configured ? 'enabled' : 'missing_api_key') : 'disabled',
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Always 2xx once the signature checks out: a retry would not change the
    // outcome, and Retell backs off after repeated failures.
